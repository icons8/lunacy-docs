...
Text is one of the key aspects of almost every design project. In this section you will learn how to work with text layers in Lunacy.
...
Lunacy applies [horizontal resize](#resize-options) to the text added this way.
...
Unlike other editors, Lunacy will at once replace all the occurrences of missing fonts throughout the entire project.
...
![Mixed settings](public/text-boldmix.png)

If a text within a layer comes in several fonts, colors, font sizes or weights (for example, one word is in bold, while the rest are in regular), the respective control will display *Mix*.
...
Alternatively, you can use the `Ctrl+Shift+R` shortcut to refresh the content.

![Generate text buttons](public/text-gen.png)
...
![Using snippets](public/text-snippets.gif)
